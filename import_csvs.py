...
def sort_files_by_date(file_list):
    # quick and very dirty natural sort algorithm for timestamps
    temp_list = [file.replace('.csv', '').split('_') for file in file_list]
    return_list = []
    months = [int(month[1]) for month in temp_list]
    # Do it per month
    for month in range(min(months), max(months) + 1):  # exclusive range, hence + 1
        current_month = [stamp for stamp in temp_list if int(stamp[1]) == month]
        # determine min and max numbers for that month
        least_day = min([int(stamp[0]) for stamp in current_month])
        largest_day = max([int(stamp[0]) for stamp in current_month])
        # rebuild the filenames in order for the returning list
        for day in range(least_day, largest_day + 1):
            return_list.append('{}_{}_2016.csv'.format(day, month))
    return return_list
